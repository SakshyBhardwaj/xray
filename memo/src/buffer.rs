...
pub trait Store: Clone {
    ...
    type FragmentStore: btree::NodeStore<Fragment, ReadError = Self::ReadError>;
    type InsertionSplitStore: btree::NodeStore<InsertionSplit, ReadError = Self::ReadError>;

    fn fragment_store(&self) -> &Self::FragmentStore;
    fn insertion_split_store(&self) -> &Self::InsertionSplitStore;

    fn local_time(&self) -> time::Local;
    fn lamport_time(&self) -> time::Lamport;
    fn recv_lamport_timestamp(&self, timestamp: time::Lamport);
}
...
{
    pub fn new(id: BufferId, store: S) -> Self {
        // Push start sentinel.
    ...
    pub fn len(&self) -> Result<usize, Error<S>> {
        Ok(self
            .fragments
            .extent::<usize, _>(self.store.fragment_store())?)
    }

    pub fn len_for_row(&self, row: u32) -> Result<u32, Error<S>> {
    ...
    pub fn longest_row(&self) -> Result<u32, Error<S>> {
        Ok(self
            .fragments
            .summary(self.store.fragment_store())?
            .longest_row)
    }

    pub fn max_point(&self) -> Result<Point, Error<S>> {
        Ok(self
            .fragments
            .extent::<Point, _>(self.store.fragment_store())?)
    }

    pub fn line(&self, row: u32) -> Result<Vec<u16>, Error<S>> {
    ...
    pub fn to_u16_chars(&self) -> Result<Vec<u16>, Error<S>> {
    ...
    pub fn cursor_at_point(&self, point: Point) -> Cursor<S> {
    ...
    pub fn add_selection_set(
        ...
        let id = self.store.local_time();
        self.selections.insert(id, set);
        self.updates.set(());
        id
    }

    pub fn remove_selection_set(&mut self, id: time::Local) -> Result<(), ()> {
        self.selections.remove(&id).ok_or(())?;
        self.updates.set(());
        Ok(())
    }

    pub fn selections(&self, set_id: time::Local) -> Result<&[Selection], ()> {
        self.selections
            .get(&set_id)
            .ok_or(())
    ...
    pub fn insert_selections<F>(&mut self, set_id: time::Local, f: F) -> Result<(), Error<S>>
    where
        F: FnOnce(&Buffer<S>, &[Selection]) -> Vec<Selection>,
    {
    ...
    pub fn mutate_selections<F>(&mut self, set_id: time::Local, f: F) -> Result<(), Error<S>>
    where
        F: FnOnce(&Buffer<S>, &mut Vec<Selection>),
    {
        let mut set = self
            .selections
            .remove(&set_id)
    ...
    fn integrate_op(&mut self, op: Arc<Operation>) -> Result<(), Error<S>> {
        self.integrate_edit(
            op.id,
            op.start_id,
            op.start_offset,
    ...
    fn integrate_edit(
        ...
        new_fragments.push_tree(cursor.slice(&old_fragments.len::<usize>(), SeekBias::Right));
    ...
    fn resolve_fragment_id(
        ...
        edit_id: time::Local,
        offset: usize,
    ) -> Result<FragmentId, Error<S>> {
        let split_db = self.store.insertion_split_store();
        ...
        cursor.seek(&offset, SeekBias::Left, split_db)?;
        Ok(cursor
            .item(split_db)?
            .ok_or(Error::InvalidOperation)?
            .fragment_id
        ...
        I: Iterator<Item = &'a Range<usize>>,
    {
        ...
        while cur_range.is_some() && cursor.item().is_some() {
            ...
            let mut fragment_start = cursor.start::<usize>().0;
                ...
                if range.end < fragment_end {
                    if range.end > fragment_start {
                        ...
                        if fragment.is_visible() {
                            prefix.deletions.insert(time::Local {
                                replica_id,
                    ...
                    version_in_range.include(&fragment.insertion);
                    if fragment.is_visible() {
                        fragment.deletions.insert(time::Local {
                            replica_id,
                            timestamp: self.local_clock,
                        });
                    }
            ...
            if let Some(range) = cur_range.clone() {
                while let Some(mut fragment) = cursor.item().cloned() {
                    fragment_start = cursor.start::<usize>().0;
    ...
    fn anchor_for_offset(&self, offset: usize, bias: AnchorBias) -> Result<Anchor, Error> {
        ...
        let offset_in_fragment = offset - cursor.start::<usize>().0;
    ...
    fn anchor_for_point(&self, point: Point, bias: AnchorBias) -> Result<Anchor, Error> {
        ...
        let offset = cursor.start::<usize>().0 + offset_in_fragment;
        self.cache_position(Some(anchor.clone()), offset, point);
        Ok(anchor)
    }
            ...
            fragments_cursor
                ...
                .map(|fragment| {
                    let overshoot = fragment
                        ...
                        .unwrap();
                    let offset = &fragments_cursor.start::<usize>().0 + &overshoot;
...
// TODO: Unify Iter and BackwardIter.
impl<S: Store> Cursor<S> {
    fn new(buffer: &Buffer<S>) -> Self {
    ...
    fn at_point(buffer: &Buffer<S>, point: Point) -> Self {
        let mut fragment_cursor = buffer.fragments.cursor();
        ...
        Self {
            fragment_cursor,
            fragment_offset,
            store: buffer.store.clone(),
        }
    }

    fn code_unit(&self) -> Result<Option<u16>, Error<S>> {
        self.fragment_cursor
            .item(self.store.fragment_store())?
            .and_then(|fragment| fragment.get_code_unit(self.fragment_offset))
    }

    fn next(&mut self) -> Result<(), Error<S>> {
        let fragment_db = self.store.fragment_store();

        self.fragment_offset += 1;
        while let Some(fragment) = self.fragment_cursor.item(fragment_db)? {
            if fragment.get_code_unit(self.fragment_offset).is_some() {
                self.fragment_offset = 0;
                break;
            } else {
                self.fragment_cursor.next(fragment_db)?;
                self.fragment_offset = 0;
            }
        }
    }

    fn prev(&mut self) -> Result<(), Error<S>> {
        let fragment_db = self.store.fragment_store();
        ...
        if self.fragment_offset > 0 {
            ...
            loop {
                self.fragment_cursor.prev(fragment_db)?;
                if let Some(fragment) = self.fragment_cursor.item(fragment_db)? {
                    if fragment.len() > 0 {
...
impl Selection {
    ...
    pub fn set_head<S: Store>(&mut self, buffer: &Buffer<S>, cursor: Anchor) {
    ...
    pub fn is_empty<S: Store>(&self, buffer: &Buffer<S>) -> bool {
...
mod tests {
    ...
    fn selections(buffer: &Rc<RefCell<Buffer>>) -> Vec<(ReplicaId, time::Local, Selection)> {
